...
export default function CropPricePage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    border: '2px solid rgba(255,255,255,0.18)', background: 'rgba(255,255,255,0.09)',
    color: '#fff', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box',
    WebkitAppearance: 'none' // Prevents iOS default styling bugs
  };
  ...
  return (
    ...
    <div dir={rtl ? 'rtl' : 'ltr'} style={{
      ...
      fontFamily: "'Noto Sans','Segoe UI',sans-serif",
      display: 'flex', flexDirection: 'column', alignItems: 'center',
      paddingBottom: 60, position: 'relative', overflowX: 'hidden'
    }}>
      ...
      {/* Main Content Area */}
      <div style={{ width:'100%', maxWidth:560, padding:'0 16px', marginTop: '20px' }}>

        {/* ── Floating Language Toggle (Moved from Nav) ── */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', position:'relative', marginBottom: '10px' }}>
          <button onClick={() => setShowLangPicker(v => !v)} style={{
            background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.22)',
            borderRadius:24, padding:'6px 14px', color:'#e8f8c8', fontSize:13,
            cursor:'pointer', display:'flex', alignItems:'center', gap:6, fontFamily:'inherit',
            backdropFilter: 'blur(5px)'
          }}>
            <span style={{ fontSize:18 }}>{cl.flag}</span>
            <span style={{ fontWeight:600 }}>{cl.native}</span>
        ...
        <div style={{ textAlign:'center', padding:'12px 0 24px' }}>
          <div style={{ fontSize:50, lineHeight:1, marginBottom:10 }}>🌾</div>
          <h1 style={{ color:'#e8f8c8', fontSize:26, fontWeight:900, margin:0,
            textShadow:'0 2px 16px rgba(0,0,0,0.5)', letterSpacing:'-0.01em' }}>{t.title}</h1>
          <p style={{ color:'#a8d870', fontSize:14, margin:'8px 0 0' }}>{t.subtitle}</p>
        </div>
        ...
        {!result && !error && (
          <div style={{ background:'rgba(255,255,255,0.07)', backdropFilter:'blur(24px)',
            ...
            <div style={{ marginBottom:16 }}>
              <label style={{ display:'block', color:'#b8e890', fontSize:13, fontWeight:700, marginBottom:8 }}>
                📍 {t.locationLabel}
              </label>
              <input value={location} onChange={e => setLocation(e.target.value)}
                placeholder={t.locationPlaceholder} style={inputStyle}
            ...
            {/* Quick chips - Mobile flexible wrap */}
            <div style={{ marginBottom:22 }}>
              <div style={{ color:'#7aaa50', fontSize:11, textTransform:'uppercase',
                letterSpacing:'0.1em', marginBottom:10 }}>{t.quickSelect}</div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                {QUICK_CROPS.map(c => (
                  <button key={c.name} onClick={() => setCrop(c.name)} style={{
                    background: crop===c.name ? 'rgba(120,210,40,0.28)' : 'rgba(255,255,255,0.07)',
                    border: `1px solid ${crop===c.name ? 'rgba(140,220,60,0.6)' : 'rgba(255,255,255,0.12)'}`,
                    borderRadius:20, padding:'6px 12px', color: crop===c.name ? '#b8f06e' : '#cde8a0',
            ...
            {/* Submit */}
            <button onClick={handleSubmit} disabled={!canSubmit} style={{
              width:'100%', padding:'16px', fontSize:16, fontWeight:800, borderRadius:16,
              border:'none', cursor: canSubmit ? 'pointer' : 'not-allowed',
              background: canSubmit
                ? 'linear-gradient(135deg,#7ac520 0%,#4a9a10 100%)'
                : 'rgba(120,180,60,0.25)',
              ...
              {loading ? (
                <>
                  <span style={{ width:20, height:20, border:'2.5px solid rgba(255,255,255,0.25)',
                    borderTopColor:'#fff', borderRadius:'50%', display:'inline-block',
                    animation:'spin 0.8s linear infinite' }} />
                  {t.checking}
                </>
        ...
        {error && (
          <div style={{ background:'rgba(220,60,60,0.12)', border:'1px solid rgba(255,100,100,0.28)',
            ...
            <div style={{ color:'#ffb0b0', fontSize:16, fontWeight:700, marginBottom:20 }}>{error}</div>
            <button onClick={() => { setError(''); setResult(null); }} style={{
              background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)',
              borderRadius:14, padding:'10px 24px', color:'#fff', fontSize:14,
              cursor:'pointer', fontFamily:'inherit', fontWeight:700,
            }}>{t.errorRetry}</button>
          </div>
        )}
        ...
        {result && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div style={{ textAlign:'center', paddingBottom:4 }}>
              <h2 style={{ color:'#b8f06e', fontSize:20, fontWeight:900, margin:0 }}>{t.resultTitle}</h2>
              <p style={{ color:'#7aaa50', fontSize:13, margin:'4px 0 0' }}>
                {result.cropName} · {result.location}
              </p>
            </div>
            ...
            <div style={{ background:'rgba(255,255,255,0.07)', backdropFilter:'blur(24px)',
              ...
              <div style={{ textAlign:'center', marginBottom:20 }}>
                <div style={{ color:'#7aaa50', fontSize:11, textTransform:'uppercase',
                  letterSpacing:'0.12em', marginBottom:4 }}>{t.avgLabel} / {result.unit}</div>
                <div style={{ color:'#e8f8c8', fontSize:44, fontWeight:900,
                  letterSpacing:'-0.02em', lineHeight:1 }}>₹{result.avgPrice}</div>
                <div style={{ marginTop:10, display:'inline-flex', alignItems:'center', gap:6,
                  background:'rgba(255,255,255,0.1)', borderRadius:20, padding:'4px 14px' }}>
                  <span style={{ fontSize:15 }}>{result.trendEmoji}</span>
                  <span style={{ color:'#c8e6a0', fontSize:13, fontWeight:600 }}>{result.trend}</span>
                </div>
              </div>

              {/* Min / Max (Responsive Grid) */}
              <div className="price-grid" style={{ display:'grid', gap:12, marginBottom:16 }}>
                {[
                  { label:t.minLabel, val:`₹${result.minPrice}`, color:'#ff9060', bg:'rgba(255,100,50,0.1)', icon:'📉' },
                  { label:t.maxLabel, val:`₹${result.maxPrice}`, color:'#7de060', bg:'rgba(80,200,40,0.1)', icon:'📈' },
                ].map(item => (
                  <div key={item.label} style={{ background:item.bg,
                    border:`1px solid ${item.color}33`, borderRadius:16, padding:'12px', textAlign:'center' }}>
                    <div style={{ fontSize:20, marginBottom:4 }}>{item.icon}</div>
                    <div style={{ color:'rgba(255,255,255,0.55)', fontSize:10,
                      textTransform:'uppercase', letterSpacing:'0.08em' }}>{item.label}</div>
                    <div style={{ color:item.color, fontSize:20, fontWeight:900, marginTop:3 }}>{item.val}</div>
                    <div style={{ color:'rgba(255,255,255,0.35)', fontSize:10 }}>per {result.unit}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background:'rgba(120,200,50,0.11)', border:'1px solid rgba(120,200,50,0.22)',
                ...
                <div style={{ color:'#a8d870', fontSize:11, textTransform:'uppercase',
                  letterSpacing:'0.09em', marginBottom:6 }}>💡 {t.adviceLabel}</div>
                <p style={{ color:'#d8f0a0', fontSize:13, lineHeight:1.6, margin:0 }}>{result.advice}</p>
              </div>
              ...
              <div style={{ background:'rgba(255,180,50,0.09)', border:'1px solid rgba(255,180,50,0.2)',
                ...
                <div style={{ color:'#f0c060', fontSize:11, textTransform:'uppercase',
                  letterSpacing:'0.09em', marginBottom:5 }}>🕐 {t.bestTimeLabel}</div>
                <p style={{ color:'#ffe8a0', fontSize:13, lineHeight:1.55, margin:0 }}>{result.bestTime}</p>
              </div>
                ...
                <div style={{ background:'rgba(80,160,240,0.09)', border:'1px solid rgba(80,160,240,0.18)',
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                    {result.nearbyMandis.map((m, i) => (
                      <span key={i} style={{ background:'rgba(80,160,240,0.14)',
                        border:'1px solid rgba(80,160,240,0.24)', borderRadius:20,
                        padding:'4px 10px', color:'#b0d8ff', fontSize:12 }}>📍 {m}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {/* Summary */}
              <div style={{ borderTop:'1px solid rgba(255,255,255,0.07)', paddingTop:14 }}>
                <p style={{ color:'rgba(200,230,160,0.75)', fontSize:13, lineHeight:1.6, margin:0 }}>
                  {result.summary}
                </p>
              </div>
            </div>
            ...
            {/* Back button */}
            <button onClick={() => { setResult(null); setError(''); }} style={{
              width:'100%', padding:'14px', fontSize:15, fontWeight:700, borderRadius:16,
              border:'2px solid rgba(255,255,255,0.18)', background:'rgba(255,255,255,0.05)',
              color:'#c8e6a0', cursor:'pointer', fontFamily:'inherit', letterSpacing:'0.02em',
            }}>← {t.tryAnother}</button>
          </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* Mobile compatibility grid styling */
        .price-grid {
