...
function FinalUi({ disable }: FinalUiProps) {
  const router = useRouter();

  // const handleViewCropAdvisory = () => {
  //   if (!disable) {
  //     router.push("/create-agri-advisor/_components/viewCropAdvisory");
  //   }
  // };
  ...
  return (
    <div className="flex flex-col items-center justify-center mt-6 p-6 bg-green-50 border border-green-200 rounded-2xl shadow-md">
      ...
      <button
        disabled={disable}
        // onClick={handleViewCropAdvisory}
        onClick={() => router.push("/view-crop-advisory")}
        className={`mt-4 w-full flex items-center justify-center gap-2 py-2 rounded-lg cursor-pointer text-white transition-all ${
          disable
